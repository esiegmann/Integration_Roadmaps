...
convert_file () {
   ...
   MY_CMD=${PANDOC_CMD[@]}
   echo "${MY_CMD[@]}"
   echo pandoc --verbose --standalone --data-dir "${PANDOC_DATADIR}" --extract-media "." --from docx --to commonmark-raw_html "$1" -o "$2/${TO_FILE}"
   pandoc --verbose --standalone --data-dir "${PANDOC_DATADIR}" --extract-media "." --from docx --to commonmark-raw_html "$1" -o "$2/${TO_FILE}"
   if [ $? -ne 0 ]; then
      echo "*** Failed: ${CMD}"
      exit $?
   else
